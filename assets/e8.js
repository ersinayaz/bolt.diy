...
window.getClosedDates = async () => {
    ...
    const responseData = {
        callerName: "closedDates",
        status: window.closedDates.length > 0 ? 200 : 401,
        error: window.closedDates.length > 0 ? false : true,
        data: window.closedDates.length > 0 ? window.closedDates : null
    };
    window.ReactNativeWebView.postMessage(JSON.stringify(responseData));
...
window.openDates = async () => {
    ...
    const responseData = {
        callerName: "openDates",
        status: oDates.length > 0 ? 200 : 401,
        error: oDates.length > 0 ? false : true,
        data: oDates.length > 0 ? oDates : null
    };
    window.ReactNativeWebView.postMessage(JSON.stringify(responseData));
    return responseData;
};
...
window.getHours = async (date, recaptchaToken) => {
    ...
    const responseData = {  
        callerName: "getHours",
        status: window.dayHours.length > 0 ? 200 : 401,
        error: window.dayHours.length > 0 ? false : true,
        data: window.dayHours.length > 0 ? window.dayHours : null
    };
    window.ReactNativeWebView.postMessage(JSON.stringify(responseData));
    return responseData;
}
