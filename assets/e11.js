    ...
    return new Promise((resolve, reject) => {
        if (window.CryptoJS) {
            try {
                ...
                const decrypted = CryptoJS.AES.decrypt(encryptedString.data, key, {
                    iv: iv,
...
window.getClosedDates = async () => {
    ...
    window.closedDates =closedDates.map(dateStr => {
    ...
    if (!window.closedDates.includes(todayStr)) {
        window.closedDates.unshift(todayStr);
    }
