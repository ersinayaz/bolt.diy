...
const request2 = (caller, endpoint, method = "GET", body = null) => {
    if (globals.debug)
    ...
    return fetch(`${globals.baseUrl}/${endpoint}`, {
        ...
        return BridgeMessage.response(caller, response.status, data);
    }).catch(err => {
...
const request = async (caller, endpoint, method = "GET", body = null) => {
    if (globals.debug)
        BridgeMessage.request(caller, body);
    ...
    .then(({ status, data }) => {
        if (globals.debug)
            return BridgeMessage.response(caller, status, data);
