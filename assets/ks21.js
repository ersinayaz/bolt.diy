...
const getHours = async (date, recaptchaToken) => {
    const [nationalityNumber, customerData] = Object.entries(config.customers)[0];
    const applicationType = Object.entries(config.customers).length == 1 ? 1 : 2;
    const response = await request(`AppointmentLayouts/GetAppointmentHourQoutaInfo?nationalityNumber=${nationalityNumber}&dealerId=${config.dealerId}&date=${date.replace(/-/g, "/")}&appointmentTypeId=${config.appointmentTypeId}&onlyAvailable=true&recaptchaToken=${recaptchaToken}&applicationType=${applicationType}`);
    const dayHours = await decrypt(response.data);
    data.hours = dayHours;
    return dayHours;
}

const sendAppointmentOTP = async () => {
    const [nationalityNumber, customerData] = Object.entries(config.customers)[0];
    return await request(`PaymentAndRefundTransactions/SendSms?customerId=${customerData.id}&nationalityNumber=${nationalityNumber}`);
}

const randomAvailableHour = () => {
    const maxIndex = data.hours.length - Object.entries(config.customers).length;
    if (maxIndex <= 0) return null;

    const availableHours = data.hours.slice(0, maxIndex + 1);
    return Math.floor(Math.random() * availableHours.length);
}

const getAppointmentHours = () => {
    const applicantLength = Object.entries(config.customers).length;
    const appointmentHours = [];
    const randomHourIndex = randomAvailableHour();
    if (randomHourIndex == null) {
        return null;
    }
    const hourIndex = globals.hours.findIndex(f => f.id == parseInt(data.hours[randomHourIndex].id));
    let availableCount = 0;
    let iterateIndex = 0;

    do {
        let hour = null;
        if (hourIndex + iterateIndex < globals.hours.length) {
            hour = globals.hours[hourIndex + iterateIndex];
        } else break;

        if (hour) {
            const availableHour = data.hours.find(f => f.appointmentHourId == hour.id);
...
const appointment = async (appointmentDate, captchaClientKey) => {
    var recaptchaToken = await solveRecaptcha(captchaClientKey);
    if (recaptchaToken == null) {
        recaptchaToken = await solveRecaptcha(captchaClientKey);
    }

    const [nationalityNumber, data] = Object.entries(config.customers)[0];
    const applicationType = Object.entries(config.customers).length == 1 ? 1 : 2;

    var body = [];

    for (const [tcKimlik, customer] of Object.entries(config.customers)) {
        const item = {
            "date": ``,
            "appointmentHourId": 1,
            "count": Object.entries(config.customers).length,
    ...
    const dayHours = await getHours(appointmentDate, recaptchaToken);
    ...
    if (appointmentHours == null) {
        ...
        return await appointment(appointmentDate,captchaClientKey);
    ...
    const response = await request(`AppointmentCustomers/Appointment?lang=tr&smsCode=${config.appointmentOTP}&paymentCustomerId=${data.id}`, "POST", body);
    return response;
}