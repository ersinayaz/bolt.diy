...
window.endPoints = {
    ...
    // "PaymentAndRefundTransactions": "PaymentAndRefundTransactions/GetPaymentInfoAppointmentCustomer2?nationalityNumber=16379251622&actionType=newAppointment",
    // "GetByTc": "Customers/GetByTC?value=16379251622&status=undefined",
    // "GetClosedDates": "AppointmentClosedDates/GetClosedDate?dealerId=1&date=2025-06-15&maxDate=2025-08-01&appointmentTypeId=16",
    // "GetHours": "AppointmentLayouts/GetAppointmentHourQoutaInfo?nationalityNumber=16379251622&dealerId=5&date=2025/06/24&appointmentTypeId=16&onlyAvailable=true&recaptchaToken=recaptchaToken&applicationType=1(bireysel)-2(aile)",
    // "SendAppointmentOTP": "PaymentAndRefundTransactions/SendSms?customerId=1687483&nationalityNumber=16379251622",
    // "PostAppointment": "AppointmentCustomers/Appointment?lang=tr&smsCode=13875&paymentCustomerId=1687483",
    // "PaymentIframeUrl": "https://api.kosmosvize.com.tr/sipay?customerBarcodes=1808654&paymentCustomerBarcode=1808654",
};
...
window.sendLoginOTP = async (people) => { // [{"tckn":"16379251622","passport":"U14684942"}]
    window.people = people;
    return await request(endPoints.SendLoginOTP, "POST", { "people": window.people });
}

window.validateLoginOTP = async (otp) => {
    const response = await request(endPoints.ValidateLoginOTP, "POST", { "people": window.people, "code": otp });
...
window.sendAppointmentOTP = async () => {
    ...
    return await request(`PaymentAndRefundTransactions/SendSms?customerId=${data.id}&nationalityNumber=${nationalityNumber}`);
}

window.getClosedDates = async () => {
    const response = await request(`AppointmentClosedDates/GetClosedDate?dealerId=${window.dealerId}&date=${window.today}&maxDate=${window.maxAppointmentDate}&appointmentTypeId=${window.appointmentTypeId}`);
...
window.appointment = async (appointmentDate) => {
    ...
    const response = await request(`AppointmentCustomers/Appointment?lang=tr&smsCode=${window.appointmentOTP}&paymentCustomerId=${data.id}`);
    return response;
}
...
window.staticRequest = async () => {
    ...
    await request(endPoints.GetDealers);
    await request(endPoints.GetCities);
    await request(endPoints.GetHours);
    await request(endPoints.GetToday);
    await request(endPoints.GetAppointmentTypes);
    await request(endPoints.GetAmount);
    await request(endPoints.GetMaxRequestCount);
    await request(endPoints.GetSmsExpirationTime);
